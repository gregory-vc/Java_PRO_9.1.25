...
1) 16.10.2025 Реализовать приложение, способное «прогонять тесты».
```
...
```
Скрипт создаст каталог `task2`, базовый `build.gradle.kts`, файл `src/main/java/App.java` (c `void main()`) и автоматически добавит `include("task2")` в `settings.gradle.kts`.
